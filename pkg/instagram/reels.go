...
func GetReelWithMetrics(ctx context.Context, mediasvc *media.Service, reelID string) (*Reel, error) {
	fields := "media_product_type,like_count,comments_count,caption,timestamp"
	mediaApiResponse, err := mediasvc.GetMediaByID(
		ctx,
		&mediaApiModel.GetMediaByIDParams{
	...
	// Only process VIDEO media (reels)
	if mediaApiResponse.Payload.MediaProductType != "REELS" {
		return nil, fmt.Errorf("media %s is not a reel", reelID)
	}
	views := int64(0)
	reach := int64(0)
	shares := int64(0)
	...
	parsedTime, err := utils.ParseTimestamp(mediaApiResponse.Payload.Timestamp)
	if err != nil {
		return nil, fmt.Errorf("could not parse timestamp for media %s: %v", reelID, err)
	}
	reel := &Reel{
		ID:                mediaApiResponse.Payload.ID,
		Views:             views,
